...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version")
		{
			...
			"A server developed manager general: DarkKonia.\n"
			"A server testers: Emma, Phantom, Myrland & others.\n";
			return false;
		}
...
void otserv(StringVec args, ServiceManager* services)
{
	...
	std::clog << STATUS_SERVER_NAME << ", version " << STATUS_SERVER_VERSION << " (" << STATUS_SERVER_CODENAME << ")" << std::endl
		...
		<< "A server developed manager general: DarkKonia." << std::endl
		<< "A server testers: Emma, Phantom, Myrland & others." << std::endl << std::endl;
	std::stringstream ss;
#ifdef __DEBUG__
	ss << " GLOBAL";
#endif
